package com.example.administrator.abbreviation;
...
import com.example.administrator.abbreviation.pickerview.OptionsPickerView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mOpv.setOnoptionsSelectListener(new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int option2, int options3) {
                // 返回的分别是三个级别的选中位置
                //mListProvince.get(options1) + mListCiry.get(options1).get(option2) +
                String tx =  mListArea.get(options1).get(option2).get(options3);
                mCity.setText(tx);
            }
        });
